...
heritability="/cellar/users/domeyer/EAGLE/heritability_tables_solid_tissues/thyroid_heritability.tsv"
...
for mode in elasticnetLDLax elasticnetLDMed elasticnetLDStrict flipAlleleLDLax flipAlleleLDMed flipAlleleLDStrict pcregressionLDLax pcregressionLDMed pcregressionLDStrict xgbLDLax xgbLDMed xgbLDStrict
do
    outdir="/cellar/shared/carterlab/projects/eagle/v0.3/thyroid/$mode"
...
echo "End Time General: $end_datetime" >> "$timing_log"
echo "---" >> "$timing_log"
echo "Total Runtime General: $(($elapsed / 3600))h $(($elapsed % 3600 / 60))m $(($elapsed % 60))s" >> "$timing_log"

# Record start time
start_time=$(date +%s)
start_datetime=$(date '+%Y-%m-%d %H:%M:%S')
echo "Finemapped Start Time: $start_datetime" >> "$timing_log"
...
for mode in elasticnetLDLax elasticnet flipAlleleLDLax flipAllele pcregressionLDLax pcregression xgbLDLax xgb
do
    outdir="/cellar/shared/carterlab/projects/eagle/v0.3/thyroid_finemapped/$mode"
...
echo "End Time Finemapped: $end_datetime" >> "$timing_log"
echo "---" >> "$timing_log"
echo "Total Runtime Finemapped: $(($elapsed / 3600))h $(($elapsed % 3600 / 60))m $(($elapsed % 60))s" >> "$timing_log"