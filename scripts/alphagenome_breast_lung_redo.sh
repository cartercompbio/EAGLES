...
cd /cellar/users/domeyer/temp_rerun_breast_lung_alphagenome
...
for tissue in breast_mammary_tissue lung
do
    

    if [[ "$tissue" == "breast_mammary_tissue" ]]; then
        ...
        echo "tissue must be  lung or breast_mammary_tissue. $tissue given."
        exit 1
    fi
