...
gtexQTLfolder="/cellar/users/domeyer/data/gtex/alphagenome_or_finemapped"
gtexQTLindexFolder="/cellar/users/domeyer/data/gtex/alphagenome_or_finemapped"
...
for tissue in whole_blood breast_mammary_tissue lung colon thyroid
do
    ...
    elif [[ "$tissue" == "colon" ]]; then
        heritability="/cellar/users/domeyer/EAGLE/heritability_tables_solid_tissues/colon_heritability.tsv"
    elif [[ "$tissue" == "thyroid" ]]; then
        heritability="/cellar/users/domeyer/EAGLE/heritability_tables_solid_tissues/thyroid_heritability.tsv"
    else
        echo "tissue must be whole_blood, lung, breast_mammary_tissue, colon or thyroid. $tissue given."
        exit 1
    fi
    
    for mode in elasticnet
    do 
        outdir="/cellar/shared/carterlab/projects/eagle/v0.3/${tissue}_alphagenome_or_finemap/$mode"
