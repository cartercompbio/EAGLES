...
NFscript="/cellar/users/domeyer/repos/EAGLES/workflows/single_gene_scores_train_only.nf"

for tissue in breast_mammary_tissue lung thyroid
do
    

    if [[ "$tissue" == "thyroid" ]]; then
        ...
        echo "tissue must be thyroid, lung or breast_mammary_tissue. $tissue given."
        exit 1
    fi
    ...
    do
        ...
        echo "-c /cellar/users/domeyer/.nextflow/config" >> "$timing_log"
        echo "-c /cellar/users/domeyer/.nextflow/config_no_resume" >> "$timing_log"
        ...
        conda run -n eagle nextflow "$NFscript" \
            -c "/cellar/users/domeyer/.nextflow/config" \
            -c "/cellar/users/domeyer/.nextflow/config_no_resume" \
            --geneInfo "$geneInfo" \
            --europfile "$europfile" \
